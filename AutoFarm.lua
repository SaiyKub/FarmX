    ...
    elseif World3 then
        ...
        elseif Lv == 2525 or Lv <= 2550 then
         ...
         elseif Lv == 2550 or Lv <= 2574 then
            ...
            CFrameQuest = CFrame.new(-16664.205078125, 105.30562591552734, 1577.4039306640625)
            CFrameMonster = CFrame.new(-16552.43359375, 99.08633422851562, 1584.231689453125)
         elseif Lv == 2575 or Lv <= 2600 then
            ...
            CFrameQuest = CFrame.new(-16664.205078125, 105.30562591552734, 1577.4039306640625)
            CFrameMonster = CFrame.new(-16883.814453125, 113.04454040527344, 1625.1070556640625)
          end
      end
  end
  function Hop()
        ...
        pcall(function()
            if _G.BringMonster then
                ...
                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if _G.AutoFarmLevel and BringFarmLevel and Nearest_Farm and v.Name == Monster and (Mon == "Factory Staff" or Mon == "Monkey" or Mon == "Dragon Crew Warrior" or Mon == "Dragon Crew Archer") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 and (v.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 275 then
                        ...
                        sethiddenproperty(game:GetService("Players").LocalPlayer,"SimulationRadius",math.huge)
                    elseif _G.AutoFarmLevel and BringFarmLevel and Nearest_Farm and v.Name == Monster and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 and (v.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= _G.BringMode then
                        v.HumanoidRootPart.Size = Vector3.new(150,150,150)
                        v.HumanoidRootPart.CFrame = PosMon
                        v.Humanoid:ChangeState(14)
                        v.HumanoidRootPart.CanCollide = false
...
local Tabs = {
    ...
    Seg = Window:AddTab({ Title = "Settings", Icon = "settings" }),
    ...
    Event = Window:AddTab({ Title = "Event", Icon = "rbxassetid://15712771666" }),
}
...
function CreatePlatform()
    ...
    platform.Size = Vector3.new(24, 0.7, 24) -- ขนาด 6x6
...
local Toggle = Tabs.Event:AddToggle("bingmob", {
    Title = "ออโต้ฟาร์มแคนดี้", 
    Description = "ใช้ได้เฉพาะ ( โลก3 )",
    Default = false,
    Callback = function(Value)
        Nearest_Farm = Value
    end
...
-- ฟังก์ชันสำหรับลบเอฟเฟกต์ที่ไม่ต้องการ
function RemoveDeathEffects()
    for _, effect in ipairs(Workspace:GetDescendants()) do
        if effect:IsA("ParticleEmitter") or effect:IsA("Light") or effect:IsA("Beam") then
            -- ตรวจสอบว่าเป็นเอฟเฟกต์ที่เกี่ยวข้องหรือไม่
            if effect.Color == ColorSequence.new(Color3.fromRGB(0, 255, 255)) or effect.Name:lower():find("death") then
                effect:Destroy()
            end
        end
    end
...
-- ใช้ลูปเพื่อตรวจสอบและลบเอฟเฟกต์ที่เกิดขึ้น
